...
export function generateNatalChart(input: BirthDataInput): NatalChart {
  const jdUt = julday(
    input.year,
    input.month,
    input.day,
  ...
  // Store all 12 house cusp longitudes (tuple index 0 = house 1, etc.)
  const houseCusps: number[] = Array.from(houses.data.houses);
  ...
  const aspects = placements.flatMap((left, leftIndex) =>
    placements.slice(leftIndex + 1).flatMap((right) => {
      ...
      const match = majorAspects.find(
        (aspect) => Math.abs(difference - aspect.angle) <= aspect.orb,
      );
      return match
        ? [{
            type:    match.type,
            between: [left.label, right.label] as [string, string],
            angle:   difference,
