...
import type { NatalChart } from '@sos/shared';
...
function formatLongitude(longitude: number) {
  const parts = split_deg(longitude, constants.SE_SPLIT_DEG_ZODIACAL | constants.SE_SPLIT_DEG_ROUND_MIN);
  return { longitude, sign: zodiac[parts.sign], degree: parts.degree, minute: parts.minute };
}
...
export function generateNatalChart(input: {
  ...
  const jdUt = julday(input.year, input.month, input.day, input.hour + input.minute / 60, constants.SE_GREG_CAL);
  ...
  const aspects = placements.flatMap((left, leftIndex) =>
    placements.slice(leftIndex + 1).flatMap((right) => {
      ...
      return match ? [{ type: match.type, between: [left.label, right.label] as [string, string], angle: difference, orb: Number(Math.abs(difference - match.angle).toFixed(2)) }] : [];
    })
  );
  ...
  return {
    metadata: {
      ...
      warnings: { houses: houses.error || null },
    },
    placements,
    angles,
    aspects,
